# AI統合クイックスタート

このガイドでは、OpenAI GPT-4oまたはAnthropic Claude 3.5 Sonnetを使用してNeuro mateのAI機能を有効にする方法を説明します。

## 📋 目次

1. [APIキーの取得](#1-apiキーの取得)
2. [環境変数の設定](#2-環境変数の設定)
3. [動作確認](#3-動作確認)
4. [本番環境へのデプロイ](#4-本番環境へのデプロイ)

## 1. APIキーの取得

### Option A: OpenAI GPT-4o（推奨）

**対応モデル:**
- ✅ `gpt-4o` - 現在推奨（高速・高品質）
- ✅ `gpt-4o-mini` - 軽量版（コスト重視）
- ✅ `gpt-5` - GPT-5リリース時に自動対応
- ✅ `o1`, `o1-mini` - 推論特化モデル

**特徴:**
- ✅ 高速（応答時間: 1-3秒）
- ✅ コスト効率良好（$0.01-0.05/相談）
- ✅ JSON出力フォーマットに対応
- ✅ 日本語の理解と生成に優れている
- ✅ **将来のモデル（GPT-5等）にも自動対応**

**取得手順:**
1. https://platform.openai.com/ にアクセス
2. アカウント作成またはログイン
3. 左メニューから「API keys」をクリック
4. 「Create new secret key」をクリック
5. キーに名前を付けて保存（例: "neuro-mate-production"）
6. 生成されたAPIキーをコピー（`sk-proj-...`で始まる文字列）
7. **重要:** このAPIキーは再表示されないため、安全に保管してください

**初期クレジット:**
- 新規アカウントには通常$5のクレジットが付与されます
- 追加クレジットは https://platform.openai.com/account/billing で購入可能

### Option B: Anthropic Claude 3.5 Sonnet

**特徴:**
- ✅ 長文理解に優れている
- ✅ 論理的思考と分析に強い
- ✅ 複雑な指示に対応可能
- ⚠️ OpenAIより若干高価（$0.015-0.06/相談）

**取得手順:**
1. https://console.anthropic.com/ にアクセス
2. アカウント作成またはログイン
3. 左メニューから「API Keys」をクリック
4. 「Create Key」をクリック
5. キーに名前を付けて保存（例: "neuro-mate-production"）
6. 生成されたAPIキーをコピー（`sk-ant-...`で始まる文字列）

**クレジット購入:**
- https://console.anthropic.com/settings/billing でクレジットを購入

## 2. 環境変数の設定

### ローカル開発環境

プロジェクトルートに`.dev.vars`ファイルを作成：

```bash
# .dev.vars ファイルの作成
cat > .dev.vars << 'EOF'
# JWT Secret（ランダムな文字列を設定）
JWT_SECRET=your-random-secret-key-here-change-this

# AI Provider（openai, anthropic, または mock）
AI_PROVIDER=openai

# OpenAI設定（OpenAIを使用する場合）
OPENAI_API_KEY=sk-proj-your-openai-api-key-here
OPENAI_MODEL=gpt-4o

# Anthropic設定（Anthropicを使用する場合）
ANTHROPIC_API_KEY=sk-ant-your-anthropic-api-key-here
ANTHROPIC_MODEL=claude-3-5-sonnet-20241022
EOF
```

**実際の値に置き換えてください:**

```bash
# OpenAI GPT-4oを使用する場合（現在推奨）
AI_PROVIDER=openai
OPENAI_API_KEY=sk-proj-ABC123...（実際のAPIキー）
OPENAI_MODEL=gpt-4o

# OpenAI GPT-5を使用する場合（リリース後）
AI_PROVIDER=openai
OPENAI_API_KEY=sk-proj-ABC123...（実際のAPIキー）
OPENAI_MODEL=gpt-5

# OpenAI o1（推論モデル）を使用する場合
AI_PROVIDER=openai
OPENAI_API_KEY=sk-proj-ABC123...（実際のAPIキー）
OPENAI_MODEL=o1

# Anthropicを使用する場合
AI_PROVIDER=anthropic
ANTHROPIC_API_KEY=sk-ant-XYZ789...（実際のAPIキー）
ANTHROPIC_MODEL=claude-3-5-sonnet-20241022
```

**モデル選択のヒント:**
- **コスト重視**: `gpt-4o-mini` または `gpt-4o`
- **品質重視**: `gpt-4o` または `gpt-5`（リリース後）
- **推論重視**: `o1` または `o1-mini`
- **長文理解**: `claude-3-5-sonnet-20241022`

### モックモード（開発用）

APIコストをかけずにテストしたい場合:

```bash
AI_PROVIDER=mock
```

モックモードでは、実際のAI APIを呼び出さず、固定のレスポンスを返します。

### 🔄 モデル切り替え（GPT-5対応）

**重要:** GPT-5やその他の新しいモデルがリリースされた際、**コード変更は一切不要**です。環境変数を変更するだけで即座に対応できます。

**切り替え手順:**

1. **`.dev.vars`ファイルを編集:**
   ```bash
   # 現在
   OPENAI_MODEL=gpt-4o
   
   # GPT-5リリース後
   OPENAI_MODEL=gpt-5
   ```

2. **サーバーを再起動:**
   ```bash
   pm2 restart neuro-mate
   ```

3. **動作確認:**
   - AI相談を実行して新しいモデルが使用されていることを確認

**本番環境でのモデル変更:**

```bash
# Cloudflare Pagesのシークレットを更新
wrangler pages secret put OPENAI_MODEL --project-name webapp
# 入力: gpt-5

# 再デプロイ（環境変数の反映）
npm run deploy
```

**対応モデル一覧:**

| モデル名 | 特徴 | 推奨用途 |
|---------|------|---------|
| `gpt-4o` | 現在推奨（高速・高品質） | 一般的な用途 |
| `gpt-4o-mini` | 軽量版（コスト削減） | 大量処理 |
| `gpt-5` | 次世代（リリース予定） | 最高品質が必要な場合 |
| `o1` | 推論特化 | 複雑な分析 |
| `o1-mini` | 推論軽量版 | コスト重視の推論 |

## 3. 動作確認

### サーバーの起動

```bash
# 依存関係のインストール（初回のみ）
npm install

# データベースのセットアップ（初回のみ）
npm run db:migrate:local

# ビルド
npm run build

# サーバー起動
pm2 start ecosystem.config.cjs

# ログ確認
pm2 logs neuro-mate --nostream
```

### AI機能のテスト

詳細なテスト手順は `test-ai-integration.md` を参照してください。

**簡単なテスト:**

```bash
# 1. ユーザー登録
curl -X POST http://localhost:3000/api/auth/signup \
  -H "Content-Type: application/json" \
  -d '{"email":"test@example.com","password":"test123"}'

# レスポンスからトークンを取得してください

# 2. AI相談テスト（トークンを置き換えてください）
curl -X POST http://localhost:3000/api/ai/consult \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer YOUR_TOKEN_HERE" \
  -d '{
    "currentConcerns": "首肩のコリと睡眠不足が気になります",
    "lifestyleRhythm": "デスクワーク中心で運動不足です",
    "additionalNotes": "サプリメントを試してみたいです"
  }'
```

**期待される動作:**
- ✅ AIが相談内容を分析してレポートを生成
- ✅ サプリメントのスコアリングと推奨理由
- ✅ セルフケアメニューの提案
- ✅ 生活習慣改善のアドバイス

**トラブルシューティング:**
- ❌ APIキーエラー: `.dev.vars`のAPIキーを確認
- ❌ プラン制限エラー: データベースでユーザーのプランを確認
- ❌ モックレスポンス: APIエラー時は自動的にモックにフォールバック

## 4. 本番環境へのデプロイ

### Cloudflare Pagesへのデプロイ

**前提条件:**
- Cloudflareアカウント
- Wrangler CLIがインストール済み（`npm install -g wrangler`）

**手順:**

1. **Cloudflare D1データベースの作成**

```bash
# 本番データベース作成
wrangler d1 create webapp-production

# 出力されたdatabase_idをwrangler.tomlに設定
# database_id = "your-database-id-here"
```

2. **環境変数（シークレット）の設定**

```bash
# 必須: JWT Secret
wrangler pages secret put JWT_SECRET --project-name webapp
# 入力: ランダムな長い文字列

# 必須: AI Provider
wrangler pages secret put AI_PROVIDER --project-name webapp
# 入力: openai または anthropic

# OpenAI使用時
wrangler pages secret put OPENAI_API_KEY --project-name webapp
# 入力: sk-proj-...

wrangler pages secret put OPENAI_MODEL --project-name webapp
# 入力: gpt-4o

# Anthropic使用時（オプション）
wrangler pages secret put ANTHROPIC_API_KEY --project-name webapp
# 入力: sk-ant-...

wrangler pages secret put ANTHROPIC_MODEL --project-name webapp
# 入力: claude-3-5-sonnet-20241022
```

3. **データベースマイグレーション**

```bash
# 本番データベースにスキーマを適用
wrangler d1 migrations apply webapp-production
```

4. **デプロイ**

```bash
# ビルドとデプロイ
npm run deploy
```

5. **確認**

デプロイが成功すると、Cloudflare PagesのURLが表示されます:
- 本番URL: `https://webapp.pages.dev`
- プレビューURL: `https://BRANCH.webapp.pages.dev`

## 💰 コスト見積もり

### OpenAI GPT-4o
- **入力:** $2.50 / 1M トークン
- **出力:** $10.00 / 1M トークン
- **1回の相談:** 約 $0.01 - $0.05
- **月間1000相談:** 約 $10 - $50

### Anthropic Claude 3.5 Sonnet
- **入力:** $3.00 / 1M トークン
- **出力:** $15.00 / 1M トークン
- **1回の相談:** 約 $0.015 - $0.06
- **月間1000相談:** 約 $15 - $60

### コスト最適化のヒント
1. **開発中はモックモード**を使用
2. **ステージング環境**でテストしてから本番デプロイ
3. **エラー処理**を適切に実装してAPI呼び出し失敗を防ぐ
4. **キャッシュ**を活用して同じ相談の再計算を避ける
5. **ログ監視**でAPI使用量を追跡

## 🔐 セキュリティベストプラクティス

1. **APIキーの保護:**
   - ❌ GitにAPIキーをコミットしない
   - ✅ `.gitignore`に`.dev.vars`を追加
   - ✅ 環境変数または秘密管理サービスを使用

2. **アクセス制御:**
   - ✅ JWT認証でAPI保護
   - ✅ プラン別のアクセス制御を実装
   - ✅ レート制限を設定

3. **監視:**
   - ✅ API使用量を定期的にチェック
   - ✅ 異常な使用パターンをアラート
   - ✅ エラーログを監視

## 📚 参考リンク

- [OpenAI Platform Documentation](https://platform.openai.com/docs)
- [Anthropic Claude Documentation](https://docs.anthropic.com/)
- [Cloudflare Workers Documentation](https://developers.cloudflare.com/workers/)
- [Hono Framework Documentation](https://hono.dev/)

## 🆘 サポート

問題が発生した場合:
1. `test-ai-integration.md`のトラブルシューティングセクションを確認
2. `pm2 logs neuro-mate`でサーバーログを確認
3. APIプロバイダーのダッシュボードで使用量とエラーを確認

---

✨ **AI統合が完了しました!** あなたのNeuro mateは、世界最先端のAIを活用してユーザーに最高のヘルスアドバイスを提供できるようになりました。
